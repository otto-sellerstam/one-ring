...
def test_raises_exception_group_from_task(run_coro, timing) -> None:
...
def test_raises_exception_group_during_start(run_coro, timing) -> None:
    def sleep_with_error(
        duration: float, error_type: type[BaseException]
    ) -> Coro[None]:
        raise error_type("Oopsie!")
...
def test_raises_exception_group_from_group(run_coro, timing) -> None:
    def sleep_with_error(
        duration: float, error_type: type[BaseException]
    ) -> Coro[None]:
        yield from sleep(0.2)

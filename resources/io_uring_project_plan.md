...
Build a Python library that provides truly asynchronous file IO by leveraging Linux's `io_uring` kernel interface, integrated with Python's `asyncio` event loop. Unlike existing libraries like `aiofiles` (which fake async by running synchronous IO in thread pools), this library will use kernel-level async IO with zero thread overhead.

Phases 1-6 cover the file IO library. Phases 7-9 extend it into network IO, culminating in an HTTP static file server built from the Linux kernel interface up.
...
- **`user_data`** — when you submit an SQE, you can tag it with an arbitrary value in the `user_data` field. When the completion (CQE) arrives, it carries that same `user_data` back. This is how you know *which* submission just completed. This concept becomes critical in Phase 3 when you need to map completions to Python `Future` objects.
...
## Phase 2: Prove the Python ↔ io_uring Plumbing
...
The goal is confidence that the plumbing works before adding the async complexity.

---

## Phase 3: Bridge io_uring to asyncio

This is the core design challenge of the whole project.

### Research

**Step 1: Understand non-blocking completion checking.**

Before jumping to asyncio, understand how to use io_uring without blocking. The key functions are:
...
The difference matters because in an async world, you never want to block. Experiment with submitting a read, doing something else, then peeking to see if it's done. This builds intuition for the async model before adding the asyncio machinery.

**Step 2: Understand asyncio internals.**
...
**Start with non-blocking polling from Python:**
- Submit a read SQE
- In a loop: peek for completions, do other work, peek again
- Prove you can do io_uring IO without ever blocking
...
- In the callback: drain the completion queue, use `user_data` from each CQE to look up the corresponding `Future`, and resolve it
- Now `await`-ing a future should properly suspend the coroutine, let other work run, and resume on completion

**Critical subtlety:** an eventfd notification is only a *hint* that completions are available. Multiple CQEs may arrive but only trigger a single eventfd notification. Your callback must drain *all* available CQEs in a loop, not just read one — otherwise completions will silently get stuck.

The Lord of the io_uring guide has a dedicated "Register an eventfd" tutorial (https://unixism.net/loti/tutorial/register_eventfd.html) — read this before starting the implementation.

**The `user_data` → `Future` mapping is the core design problem.** When you submit an SQE, you assign it a `user_data` value and store a `Future` keyed by that value. When a CQE arrives, you read its `user_data`, find the matching `Future`, and call `future.set_result()` with the result. This is what connects io_uring's completion model to Python's async/await.
...
## Phase 6: Replace the FFI Layer with Your Own Bindings
...
Once the file IO library is complete, the natural extension is network IO — io_uring handles sockets through the same SQ/CQ interface, using operations like `io_uring_prep_accept`, `io_uring_prep_recv`, and `io_uring_prep_send` instead of file read/write. Your asyncio bridge, `user_data` → `Future` mapping, and overall architecture all carry over directly.

### Phase 7: TCP Echo Server with io_uring
...
- The `05_webserver_liburing` example from io_uring-by-example and the "A web server with liburing" tutorial from Lord of the io_uring — both build a C web server on io_uring and are directly relevant here
...
### Phase 8: HTTP/1.1 Request Parsing
...
- You'll hit the same buffering challenge as `readline()` from Phase 4 — TCP delivers a stream of bytes with no message boundaries, so a single `recv` might contain half a request or two and a half requests. You need to buffer and scan for `\r\n\r\n`.
- Generate HTTP responses: status line, headers, body. A minimal "hello world" response is just a hardcoded string.

### Phase 9: Static File Server
...
| io_uring-by-example | Hands-on C examples (includes webserver) | Phases 1, 7 |
| loti-examples | Companion code for Lord of the io_uring (eventfd, fixed buffers, etc.) | Phases 1, 3 |
| `liburing` (PyPI) | Python API over io_uring | Phase 2 |
| `aioring` | API design, asyncio integration | Phases 3-4 |
| `uring_file` | Minimal asyncio bridge | Phase 3 |
| `Shakti` | Python async/await interface over liburing | Phases 3-4 |
| `kloop` | Full event loop replacement | Phase 3 |
| `tokio-uring` (Rust) | Completion-based async runtime design | Phase 3 |
| `uvloop` | Non-standard IO backend + asyncio | Phase 3 |
| `aiofiles` | User-facing API expectations | Phase 4 |
| `liburing` (C, GitHub) | FFI-friendly shared library | Phase 6 |
| liburing networking wiki | io_uring socket operations | Phase 7 |
| RFC 9112 | HTTP/1.1 message syntax | Phase 8 |
...
- **`user_data` and completion tracking** — mapping kernel completions to application state (Phases 1, 3)
- **Linux syscall mechanics** — how system calls work and why minimizing them matters (Phases 1-2)
- **epoll internals** — since you'll integrate with it via asyncio (Phase 3)
- **asyncio event loop architecture** — moving from "user" to "implementer" understanding (Phase 3)
- **API design** — making low-level primitives feel Pythonic (Phase 4)
- **FFI and native extensions** — calling C from Python, memory ownership, GIL management (Phase 6)
- **Abstraction design** — your FFI migration will test whether your internal boundaries are clean (Phase 6)
- **TCP/IP socket programming** — how connections are established and data flows at the syscall level (Phase 7)
- **Protocol parsing** — turning a raw byte stream into structured HTTP messages (Phase 8)
- **Unified IO architecture** — using a single event loop for both disk and network IO (Phase 9)
